...
class ReadReceiptsScreen extends StatelessWidget {
  const ReadReceiptsScreen({super.key});
    ...
    return BlocBuilder<PrivacyCubit, PrivacyState>(
      builder: (context, state) {
        if (state.state == CubitState.failure) {
          ...
          return ReadReceiptsPage(state: state);
        }
        return ReadReceiptsPage(state: state);
      },
    );
...
class ReadReceiptsPage extends StatelessWidget {
  final PrivacyState state;
  const ReadReceiptsPage({super.key, required this.state});

  @override
  Widget build(BuildContext context) {
    void saveSettings(String? value) async {
      final cubit = context.read<PrivacyCubit>();
      await cubit.updatePrivacySettings(newEnableReadReceipt: value);
      cubit.loadPrivacySettings();
    }

    return Scaffold(
      appBar: CAppBar(
        title: const Text(AppStrings.readReceipts),
        ...
        Column(
          children: [
            RadioTile(
                ...
                groupValue: state.enableReadReceipt),
            const Divider(
              thickness: 0.5,
            ),
            RadioTile(
